...
| `lib/mcp/guardrails.ts`        | Code-level enforcement: confirmation gate on irreversible calls, one-success-per-turn latch on placement, required-arg validation, ₹1000 cap, ₹99 min, no blind retry of placement (check-then-retry), 10s track cooldown, phantom-coupon scrubbing, address-id validation, add-on-id rejection, whole-cart merge on `update_food_cart`, automatic coupon re-apply, price lock at placement, tool-call log |
| `lib/mcp/facts.ts`             | Conversation facts replayed from history each turn (saved address ids, menu vs add-on ids, last cart and its payloads, applied coupon, last total quoted to the user) - what the guards above check against                                                                                                                                                                                                |
