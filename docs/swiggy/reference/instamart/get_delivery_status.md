...
- Use for structured delivery ETA refreshes after an Instamart order is placed.
- Do not call in a tight loop. Wait for `pollIntervalSec`, and stop when `cancelled` or `delivered` is `true`.
- For a user-facing conversational tracking answer, prefer `track_order`.
...
This schema documents the structured payload returned by `get_delivery_status`. Optional fields can vary by user state, cart state, and live Swiggy availability.
